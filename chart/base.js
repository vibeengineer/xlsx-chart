...
var Chart = Backbone.Model.extend({
  ...
  writeTable: function (cb) {
    ...
    me.read({ file: "xl/worksheets/sheet2.xml" }, function (err, o) {
      ...
      o.worksheet.dimension.$.ref =
        "A1:" + me.getColName(me.titles.length + 1) + (me.fields.length + 1);
      var rows = [
        {
          $: {
            r: 1,
  ...
  writeMultTable: function (row, cb) {
    ...
    me.read({ file: "xl/worksheets/sheet2.xml" }, function (err, o) {
      ...
      o.worksheet.dimension.$.ref = `A${row}:${me.getColName(me.titles.length + 1)}${
        me.fields.length + row + 1
      }`;
  ...
  writeChart: function (chartN, row, cb) {
    ...
    me.read({ file: "xl/charts/chart1.xml" }, function (err, o) {
      ...
      _.each(me.titles, function (t, i) {
        ...
        if (chartOpts.customColors) {
          ...
          if (customColors.points) {
            customColorsPoints["c:dPt"] = chartOpts.fields
      ...
      const templateChartName =
        "c:" +
        CHART_TYPES.find(
          (chartType) =>
            me.chartTemplate["c:chartSpace"]["c:chart"]["c:plotArea"][`c:${chartType}Chart`]
        ) +
        "Chart";
      ...
      _.each(seriesByChart, function (ser, chart) {
        ...
        o["c:chartSpace"]["c:chart"]["c:plotArea"][chartTagName] =
          o["c:chartSpace"]["c:chart"]["c:plotArea"][chartTagName] || [];
        ...
        if (chart == "column" || chart == "bar") {
          ...
          newChart = _.clone(
            me.chartTemplate["c:chartSpace"]["c:chart"]["c:plotArea"][templateChartName]
          );
          newChart["c:barDir"] = {
            $: {
        ...
        if (chartOpts.manualLayout && chartOpts.manualLayout.plotArea) {
          o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:layout"] =
            o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:layout"] || {};
      ...
      // Add chart background color support
      if (chartOpts.chartBackground) {
        o["c:chartSpace"]["c:spPr"] = {
          "a:solidFill": {
            "a:srgbClr": {
              $: {
                val: chartOpts.chartBackground,
              },
            },
          },
        };
      ...
      // Add plot area background color
      if (chartOpts.plotAreaBackground) {
        o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:spPr"] = {
          "a:solidFill": {
            "a:srgbClr": {
              $: {
                val: chartOpts.plotAreaBackground,
              },
            },
          },
        };
      ...
      // Add axis text color support
      if (chartOpts.textColor) {
        // Category axis text color
        if (o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:catAx"]) {
          const catAxes = Array.isArray(o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:catAx"])
            ? o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:catAx"]
            : [o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:catAx"]];

          catAxes.forEach((axis) => {
            axis["c:txPr"] = {
              "a:bodyPr": {},
              "a:lstStyle": {},
              "a:p": {
        ...
        if (o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:valAx"]) {
          const valAxes = Array.isArray(o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:valAx"])
            ? o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:valAx"]
            : [o["c:chartSpace"]["c:chart"]["c:plotArea"]["c:valAx"]];

          valAxes.forEach((axis) => {
            axis["c:txPr"] = {
              "a:bodyPr": {},
              "a:lstStyle": {},
              "a:p": {
      ...
      // Add legend text color support
      if (chartOpts.textColor && o["c:chartSpace"]["c:chart"]["c:legend"]) {
        o["c:chartSpace"]["c:chart"]["c:legend"]["c:txPr"] = {
          "a:bodyPr": {},
          "a:lstStyle": {},
          "a:p": {
            "a:pPr": {
          ...
          "a:p": {
            ...
            "a:r": {
              "a:rPr": {
                ...
                ...(chartOpts.titleColor && {
                  "a:solidFill": {
                    "a:srgbClr": {
                      $: {
                        val: chartOpts.titleColor,
                      },
                    },
                  },
                }),
    ...
    async.series(
      [
        function (cb) {
          ...
          let path = me.templatePath
            ? me.templatePath
            : __dirname + "/../template/" + me.tplName + ".xlsx";
      ...
      [
        ...
        function (cb) {
          ...
          async.eachSeries(
            me.charts,
            (chart, cb) => {
              ["chart", "titles", "fields", "data", "chartTitle"].forEach(
                (a) => (me[a] = chart[a])
              );
                ...
                [
                  ...
                  function (cb) {
                    me.read({ file: "xl/drawings/drawing1.xml" }, function (err, o) {
                      ...
                      if (n == 1) {
                        o["xdr:wsDr"]["xdr:twoCellAnchor"]["xdr:from"]["xdr:col"] =
                          position.fromColumn;
                        o["xdr:wsDr"]["xdr:twoCellAnchor"]["xdr:from"]["xdr:colOff"] =
                          position.fromColumnOffset;
                        o["xdr:wsDr"]["xdr:twoCellAnchor"]["xdr:from"]["xdr:row"] =
                          position.fromRow;
                        o["xdr:wsDr"]["xdr:twoCellAnchor"]["xdr:from"]["xdr:rowOff"] =
                          position.fromRowOffset;
                        o["xdr:wsDr"]["xdr:twoCellAnchor"]["xdr:to"]["xdr:col"] = position.toColumn;
                        o["xdr:wsDr"]["xdr:twoCellAnchor"]["xdr:to"]["xdr:colOff"] =
                          position.toColumnOffset;
                        o["xdr:wsDr"]["xdr:twoCellAnchor"]["xdr:to"]["xdr:row"] = position.toRow;
                        o["xdr:wsDr"]["xdr:twoCellAnchor"]["xdr:to"]["xdr:rowOff"] =
                          position.toRowOffset;
